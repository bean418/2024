...
df_co2 <- as.data.frame(lapply(df_co2[120:137, 2:81],
                               as.numeric))
df_gas <- as.data.frame(lapply(df_gas[120:137, 2:81],
                               as.numeric))
...
#plot
plot(1:18, df_co2$BD3_SEN5)
plot(1:18, df_gas$BD3_SEN5)
...
for (i in 1:39) {
  noisy_data <- data.frame(co2 = add_noise( (df_co2$co2+5000) ))
  df_list[[i+1]] <- noisy_data
}
df_list[[1]] <- data.frame(co2 = add_noise(df_co2$co2+5000))
